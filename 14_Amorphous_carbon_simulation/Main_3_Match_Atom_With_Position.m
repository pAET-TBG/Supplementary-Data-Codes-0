...
atom_info = importdata('atom_tracing_model.mat');
atompos   = atom_info.atom;
label     = atom_info.label;
label(:)  = 1;
clear atom_info 

projs = My_paddzero(projs,[180,180,37]);
atompos(1,:) = atompos(1,:) - mean(atompos(1,:)) + 2/3;
atompos(2,:) = atompos(2,:) - mean(atompos(2,:)) + .5;
atompos(3,:) = atompos(3,:) - mean(atompos(3,:));
res = 0.339673913043478;
...
for tempi = 1:size(projs,3)
    ...
    tempCP = imgaussfilt(tempCP,1);
    tempCP(tempCP>0) = 1;  
    cropProjs(:,:,tempi) = tempCP.*projs(:,:,tempi);
end
for tempi = 1:size(cropProjs,3)
    tempProjs = cropProjs(:,:,tempi);
    tempProjs(tempProjs>0) = tempProjs(tempProjs>0) - min(min(min(tempProjs(tempProjs>0))));
    cropProjs(:,:,tempi) = tempProjs;
    clear tempProjs
end
%% Fit projections
sumVector = zeros(1,size(cropProjs,2));
for tempi = 1:size(cropProjs,3)
    sumVector = sumVector+sum(cropProjs(:,:,tempi),1);
end
sumVector = sumVector./size(cropProjs,3);
for tempi = 1:size(cropProjs,3)
    for iter = 1:3
        [p, ~, ~, ~] = My_1Dcurvefit_Scale(sumVector, sum(cropProjs(:,:,tempi),1),1);
        cropProjs(:,:,tempi) = cropProjs(:,:,tempi).*p;
    end
end
