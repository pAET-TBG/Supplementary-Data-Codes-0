...
template <typename T>
void __global__ updateRec(T*rec_new, const T* coeffs, int dimx, int dimy, long long N, long long starting_pt){
    long long const i = blockDim.x * blockIdx.x + threadIdx.x;
    if (i<N ) {
        long long ii = i+starting_pt;
        const int index_z =  int( ii/(dimx*dimy) ) ;
        const T coeff = coeffs[index_z];

        rec_new[i] = max( 0.0, coeff*rec_new[i] );
    }
}
...
void mexFunction(int nlhs, mxArray *plhs[],int nrhs, const mxArray *prhs[]) {
    ...
    */
    ...
    float const  * coeffs       = mxGetSingles(prhs[6]);
    ...
    float ** d_coeffs   = new float*[deviceCount]; 
    ...
    */
    ...
    for(int device=0; device<deviceCount; device++) {
        ...
        cudaMalloc( &d_coeffs[device], dimz*sizeof(float) );
        cudaMemcpy(  d_coeffs[device], coeffs + dimz_i*0, dimz*sizeof(float), cudaMemcpyHostToDevice );
    }
    ...
    for (long iter=0; iter<iterations; iter++){
        ...
        if(positivity){
            for(int count=0; count<deviceCount; count++) {
                cudaSetDevice(count);
                updateRec<<<blocksPerGridRec_i, threadsPerBlock>>>(d_rec[count], d_coeffs[count], dimx, dimy, recPoints_i, recPoints_i*count);
            }
        }
