...
% use 6 for type 1, 14 for type 2; (C: 6 Si: 14)
Z_arr   = [6, 14];  
% Z_arr   = [6];    
...
para0 = [1 1.5;  
         3.5 3.5];                                                         % the initial parameter for the fitted function
lb = [0   0;
      3  3];                                                               % the lower bound of each paremeter
ub = [2  4;
      4  4];                                                               % the upper bound of each parameter
...
for jjjj=1:10
    ...
    xdata.step_sz    = 1;
    xdata.iterations = 10;
    [~,para0,~] = gradient_B_2type_difB(para0, xdata, projections);
    
    xdata.step_sz    = 10*(0.99^(jjjj-1));
    xdata.iterations = 50;
    [~,para,~] = gradient_fixHB_XYZ(para0, xdata, projections); 
    model = para(3:5,:);
end
save([pwd,'/output_data/atom_tracing_model_refinement_total.mat'],'model')

%% calculate the B factor
calculate_B_factor(para0, xdata, model, projections);

%% crop the refinement result and calculate the manual tracing ratio
clear;clc
...
atomDist = 1.9;                                                            % atom distance
VarTh = 1.42/4;
atom(1,:) = atom(1,:)./Res+812;
atom(2,:) = atom(2,:)./Res+737;
atom(3,:) = atom(3,:)./Res+40;

%% crop the best auto tracing region
load('Index_For_Cropped_Atom.mat')
atom        = atom(:,index==1);                                            % Initialize an empty array for atom position
label       = label(index==1);                                             % Initialize an empty array for label
manualLabel = manualLabel(index==1);                                       % Initialize an empty array for calculationg manual tracing ratio
fprintf(['The Ratio of Manual Tracing: ', num2str(sum(manualLabel==2)/size(manualLabel,2)), ' \n'])
CentralPixels_Origin = [823.5000  721.5000   40.5000];
atom = atom - CentralPixels_Origin';
atom = atom.*Res;
save([pwd,'/output_data/atom_tracing_model_refinement.mat'],'atom','label');
